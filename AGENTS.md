...
- **AD10**: The importer is the actor for subscription/order creation and may call the WC Subscriptions API directly (e.g. `WicketImporter\BulkImport\Subscriptions\SubscriptionCreator`). The `ImportAdapter` path still fires `wicket_import_create_subscription` so extensions can react/adjust — not create.
